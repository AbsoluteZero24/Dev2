...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $additional_data = array(), $groups = array(), $places = array(), $modules = array())
	{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE){
				...
				$places = $this->ion_auth->get_users_places($user->id)->result();				
				$place_id = (!empty($places['0']->id)) ? $places['0']->id : '' ;
				$place_name = (!empty($places['0']->name)) ? $places['0']->name : '';
				
				$modules = $this->ion_auth->get_users_modules($user->id)->result();				
				$module_id = (!empty($modules['0']->id)) ? $modules['0']->id : '' ;
				$module_name = (!empty($modules['0']->name)) ? $modules['0']->name : '';
				
				$groups = $this->ion_auth->get_users_groups($user->id)->result();				
				$group_id = (!empty($groups['0']->id)) ? $groups['0']->id : '' ;
				$group_name = (!empty($groups['0']->name)) ? $groups['0']->name : '';
	...
	public function update($id, array $data)
	{
		...
		/*MasnaV by Rofiq*/
		 if (array_key_exists('group', $data_full)) {	
				$group['group_id'] =  $data_full['group'];
				$this->db->update($this->tables['users_groups'], $group, array('user_id' => $user->id));
		 }
		 if (array_key_exists('place', $data_full)) {	
				$place['place_id'] =  $data_full['place'];
				$this->db->update($this->tables['users_places'], $place, array('user_id' => $user->id));
		 }
		 
		  if (array_key_exists('module', $data_full)) {	
				$module['module_id'] =  $data_full['module'];
				$this->db->update($this->tables['users_modules'], $module, array('user_id' => $user->id));
		 }
	...
	**/
	...
	/****** Masnav Start *****/
	 public function get_users_places($id=FALSE)
	{
		$this->trigger_events('get_users_place');

		//if no id was passed use the current users id
		$id || $id = $this->session->userdata('user_id');

		return $this->db->select($this->tables['users_places'].'.'.$this->join['places'].' as id, '.$this->tables['places'].'.name, '.$this->tables['places'].'.description')
		                ->where($this->tables['users_places'].'.'.$this->join['users'], $id)
		                ->join($this->tables['places'], $this->tables['users_places'].'.'.$this->join['places'].'='.$this->tables['places'].'.id')
		                ->get($this->tables['users_places']);
	}
	 ...
	 * @author Ben Edmunds
	 **/
	public function add_to_place($group_id, $user_id=false)
	{
		$this->trigger_events('add_to_place');

		//if no id was passed use the current users id
		$user_id || $user_id = $this->session->userdata('user_id');

		return $this->db->insert($this->tables['users_places'], array( $this->join['places'] => (int)$group_id, $this->join['users'] => (int)$user_id));
	}
	 ...
	 * @author Ben Edmunds
	 **/
	public function remove_from_place($group_ids=false, $user_id=false)
	{
		$this->trigger_events('remove_from_place');
		...
		if( ! empty($group_ids))
		{
			if(is_array($group_ids))
			{
				foreach($group_ids as $group_id)
				{
					$this->db->delete($this->tables['users_places'], array($this->join['places'] => (int)$group_id, $this->join['users'] => (int)$user_id));
				}
			...
			{
				return $this->db->delete($this->tables['users_places'], array($this->join['places'] => (int)$group_ids, $this->join['users'] => (int)$user_id));
			}
		}
		// otherwise remove user from all groups
		else
		{
			return $this->db->delete($this->tables['users_places'], array($this->join['users'] => (int)$user_id));
		}
	}
	...
	public function places()
	{
		...
		$this->response = $this->db->get($this->tables['places']);
	...
	{
		$this->trigger_events('place');
		if (isset($id))	{
			$this->db->where($this->tables['places'].'.id', $id);
		}
		$this->limit(1);
		return $this->places();
	}
	...
	public function get_users_modules($id=FALSE)	{
		$this->trigger_events('get_users_modules');
		//if no id was passed use the current users id
		$id || $id = $this->session->userdata('user_id');

		return $this->db->select($this->tables['users_modules'].'.'.$this->join['modules'].' as id, '.$this->tables['modules'].'.name, '.$this->tables['modules'].'.description')
		                ->where($this->tables['users_modules'].'.'.$this->join['users'], $id)
		                ->join($this->tables['modules'], $this->tables['users_modules'].'.'.$this->join['modules'].'='.$this->tables['modules'].'.id')
		                ->get($this->tables['users_modules']);
	}
	
	 public function add_to_module($module_id, $user_id=false){
		$this->trigger_events('add_to_module');
		//if no id was passed use the current users id
		$user_id || $user_id = $this->session->userdata('user_id');

		return $this->db->insert($this->tables['users_modules'], array( $this->join['modules'] => (int)$module_id, $this->join['users'] => (int)$user_id));
	}  
	
	public function modules()
	{
		$this->trigger_events('modules');
		...
		$this->response = $this->db->get($this->tables['modules']);
	...
	{
		$this->trigger_events('module');
		if (isset($id))	{
			$this->db->where($this->tables['modules'].'.id', $id);
		}
		$this->limit(1);
		return $this->modules();
	}
