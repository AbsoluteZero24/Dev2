...
foreach ($menu as $row) {
    if ($row->level == 0) {
        ...
        // menu separator
        echo '<li class="header">' . $row->label . '</li>';

        $menu_01 = $this->auth_menus_model->get_parent(10, $row->id);

        foreach ($menu_01 as $row_01) {
            if ($row_01->level == 1) {
                if (is_null($row_01->url) || empty($row_01->url) || $row_01->url == '') {
                    $ref = '';
                } else {
                    $ref = site_url() . '/' . $row_01->url;
                }
                ...
                if ($row_01->jml_child == 0) {
                    ...
                    foreach ($menu_02 as $row_02) {
                        if (is_null($row_02->url) || empty($row_02->url) || $row_02->url == '') {
                            $ref = '';
                        } else {
                            $ref = site_url() . '/' . $row_02->url;
                        }
