...
class Ion_auth {
     ...
     * @author Mathew
     * */
    public function register($username, $password, $email, $additional_data = array(), $group_name = array(), $place_name = array(), $modules_name = array()) { //need to test email activation
        $this->ion_auth_model->trigger_events('pre_account_creation');

        $email_activation = $this->config->item('email_activation', 'ion_auth');

        if (!$email_activation) {
            $id = $this->ion_auth_model->register($username, $password, $email, $additional_data, $group_name, $place_name, $modules_name);
            if ($id !== FALSE) {
    ...
    public function logout() {
        ...
        $this->session->unset_userdata('place_name');
    ...
    public function logged_in() {
        ...
        $identity = $this->config->item('identity', 'ion_auth', 'place');
    ...
    public function is_operator() {
        ...
        return $this->in_group($operator_group);
    }

    public function is_report() {
        $this->ion_auth_model->trigger_events('is_report');

        $report_group = $this->config->item('report_group', 'ion_auth');

        return $this->in_group($report_group);
    }
    ...
    /** MasnaV * */
    public function in_place($check_place, $id = false) {
        $this->ion_auth_model->trigger_events('in_place');

        $users_places = $this->ion_auth_model->get_users_places($id)->result();
        $places = array();
        foreach ($users_places as $place) {
            $places[] = $place->name;
        }

        if (is_array($check_place)) {
            foreach ($check_place as $key => $value) {
                if (in_array($value, $places)) {
                    return TRUE;
                }
            }
        } else {
            if (in_array($check_place, $places)) {
                return TRUE;
            }
        }
    ...
    public function in_module($check_module, $id = false) {
        $this->ion_auth_model->trigger_events('in_module');

        $users_modules = $this->ion_auth_model->get_users_modules($id)->result();
        $modules = array();
        ...
        if (is_array($check_module)) {
            ...
            if (in_array($check_module, $modules)) {
                return TRUE;
            }
        }
