...
class My_Model extends CI_Model {
	...
	public function database($database){
		$this->db = $this->load->database('sdm_current', true);
		$_database = $this->load->database('sdm_current', true);
	}
    ...
    /**
     * Fetch an array of records based on an arbitrary WHERE LIKE -- OR call. -ROFIQ
     * modified by sen
     */
    public function get_like_by_or() {
        $where = func_get_args();
        //$this->_set_like_or($where);
        $this->_set_like_or_sen($where);
        return $this->get_all();
    }

    /**
     * Fetch an array of records based on an arbitrary WHERE LIKE -- AND call. -ROFIQ
     */
    public function get_like_by_and() {
        $where = func_get_args();
    ...
    public function update($primary_value, $data, $skip_validation = FALSE) {
        ...
        if ($data !== FALSE) {
            $result = $this->_database->where($this->_primary_key, $primary_value)
                    ->set($data)
                    ->update($this->_table);
    ...
    public function update_many($primary_values, $data, $skip_validation = FALSE) {
        ...
        if ($data !== FALSE) {
            $result = $this->_database->where_in($this->_primary_key, $primary_values)
                    ->set($data)
                    ->update($this->_table);
    ...
    public function delete_many($primary_values) {
        ...
        $this->_database->where_in($this->_primary_key, $primary_values);
    ...
    public function relate($row) {
        ...
        foreach ($this->has_many as $key => $value) {
            ...
            if (in_array($relationship, $this->_with)) {
                ...
                if (is_object($row)) {
                    $row->{$relationship} = $this->{$relationship . '_model'}->get_many_by($options['primary_key'], $row->{$this->_primary_key});
                } else {
                    $row[$relationship] = $this->{$relationship . '_model'}->get_many_by($options['primary_key'], $row[$this->_primary_key]);
                }
            }
    ...
    /**
     * Fetch a count of rows based on an arbitrary WHERE call on LIKE(AND) Condition  . -- ROFIQ 
     */
    public function count_by_like_and() {
        if ($this->soft_delete && $this->_temporary_with_deleted !== TRUE) {
            $this->_database->where($this->soft_delete_key, (bool) $this->_temporary_only_deleted);
        }
    ...
    /**
     * Fetch a count of rows based on an arbitrary WHERE call on LIKE(OR) Condition  . -- ROFIQ 
     */
    public function count_by_like_or() {
        if ($this->soft_delete && $this->_temporary_with_deleted !== TRUE) {
            $this->_database->where($this->soft_delete_key, (bool) $this->_temporary_only_deleted);
        }
    ...
    private function _fetch_primary_key() {
        if ($this->_primary_key == NULl) {
            $this->_primary_key = $this->_database->query("SHOW KEYS FROM `" . $this->_table . "` WHERE Key_name = 'PRIMARY'")->row()->Column_name;
        }
    }
    ...
    /**
     * Set WHERE parameters, cleverly - ROFIQ
     */
    protected function _set_like_or($params) {
        if (count($params) == 1 && is_array($params[0])) {
            foreach ($params[0] as $field => $filter) {
                $this->_database->or_like($field, $filter);
            }
        } else if (count($params) == 1) {
            ...
            $this->_database->or_like($params[0], $params[1], $params[2]);
        } else {
    ...
    //by sen
    protected function _set_like_or_sen($params) {
        if (count($params) == 1 && is_array($params[0])) {
            foreach ($params[0] as $field => $filter) {
                if (!empty($filter) or $filter!=0 or $filter!="") {
                    $this->_database->or_like($field, $filter);
                }
            }
        } else if (count($params) == 1) {
...
//    --------------- by seno --------------------
    /* by seno */
    public function get_fields_name() // seno
    {
        $nama_fields = $this->_database->list_fields($this->_table);
        return $nama_fields;
    }

    public function get_fields_info() {
        $sql = 'select column_name, column_comment, is_nullable from information_schema.columns a where table_name = "'.$this->_table.'"';
        return $this->_database->query($sql)->result_array();
//
//'
//        
//        return $this->_database->select('column_comment')
//                        ->from('information_schema.columns')
//                        ->where('TABLE_NAME', $this->_table)->get()->row_array();
////        return (int) $this->_database->select('column_comment')
////                        ->from('information_schema.columns')
////                        ->where('TABLE_NAME', $this->_table)->get()->row_array();
    }

    public function insert_batch($data) // seno
    {
        $this->db->insert_batch($this->_table,$data);
        
    }
    public function update_batch($data) // seno
    {
        $this->db->update_batch($this->_table,$data,$this->_primary_key);
    }

    public function field_data() // seno
    {
        $field_data = $this->_database->field_data($this->_table);
        return $field_data;
    }
