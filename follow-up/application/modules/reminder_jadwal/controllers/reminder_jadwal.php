...
class reminder_jadwal extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->load->model(array(
            'master/trx_reminder_jadwal_model',
            'reference_model'
        ));
        $this->form = 'reminder_jadwal';
    }
...
//
        $data['data'] = $this->trx_reminder_jadwal_model->get_all();
...
// -- loading tabel utama --
            $main_row = $this->trx_reminder_jadwal_model->as_array()->get($id);
            $data += reformat_main_row_array($this->trx_reminder_jadwal_model, $main_row);


// -- loading tabel child --
//$data_survei = $this->trx_reminder_jadwal_model->get_many_by('id', $data['id']);
...
// -- update tabel utama ---------
            if ($id == '') {
                ...
                $id = $this->trx_reminder_jadwal_model->insert($post);
            } else {
                $post['usr_upd'] = $this->session->userdata('user_id');
                $post['tgl_upd'] = date('Y-m-d', now()) . ' ' . date('H:i:s', time());
                $this->trx_reminder_jadwal_model->update($id, $post);
            }
...
//            }
    ...
    function delete() {
        ...
        $this->trx_reminder_jadwal_model->delete($id);
        redirect($this->form);
    }
...
//$document = $PHPWord->loadTemplate('d:\draft_ls.docx');
        ...
        $data = $this->trx_reminder_jadwal_model->get_data($tgl);
//        echo '<pre>';
//        print_r($data);
//        return;
